...
def sort_cluster(HTSfile,idmin ):
	...
	input = open('results'+HTSfile.split('.fa')[0]+'.uc','r')
	for row in input:
		if row.split('\t')[0] == 'H':
			cluster = row.split('\t')[1]
			masterseq.append(row.split('\t')[9].split('\n')[0])
			if row.split('\t')[9].split('\n')[0] not in mastersequnique:
				mastersequnique.append(row.split('\t')[9].split('\n')[0])
			print(cluster, "Idem at ", idmin*100 , " : ", row.split('\t')[8], row.split('\t')[9].split('\n')[0])
		if row.split('\t')[0] == 'S':
			if row.split('\t')[8].split('\n')[0] not in mastersequnique:
				mastersequnique.append(row.split('\t')[8].split('\n')[0])
				masterseq.append(row.split('\t')[8].split('\n')[0])
			
	outrided = open(HTSfile.split('.')[0]+'_ridcluster.fasta', 'w+')
	for seq in mastersequnique:
		masterseqrided[seq] = int(masterseq.count(seq))
		outrided.write('>'+seq+';size='+str(masterseqrided[seq]) + '\n'+ str(fastadict[seq]) + '\n')
